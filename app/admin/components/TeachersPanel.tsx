  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-800">
        <h2 className="text-white font-bold text-lg">Eğitmenler</h2>
        <button
      ...
      {adding && (
        <TeacherForm onSubmit={handleAdd} onCancel={() => setAdding(false)} label="Ekle" />
      )}
          ...
          editingId === teacher.id ? (
            ...
            <div key={teacher.id} className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => setEditingId(teacher.id)} className="text-xs text-zinc-400 hover:text-amber-400 transition-colors">Düzenle</button>
                <button onClick={() => handleDelete(teacher.id)} className="text-xs text-zinc-400 hover:text-red-400 transition-colors">Sil</button>
              </div>
            </div>
          )
        )}
...
function TeacherForm({ defaults, onSubmit, onCancel, label }: {
  defaults?: Teacher;
  ...
  return (
    <form onSubmit={onSubmit} className="bg-zinc-900 border border-amber-400/30 rounded-xl p-4 flex flex-col gap-3 mb-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-zinc-400 text-xs mb-1">Ad Soyad</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-zinc-400 text-xs mb-1">Instagram Kullanıcı Adı</label>
          <input name="ig_handle" defaultValue={defaults?.ig_handle} required className={inputCls} />
        </div>
        <div>
          <label className="block text-zinc-400 text-xs mb-1">Baş Harfler</label>
