...
export default function ContactPhoneDialog({
  ...
  return (
    <div className="group relative w-full h-[140px]">
      <style>{`
        @keyframes flipIn {
          0% {
        ...
        .flip-card-front.reverse {
          animation: flipOut 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards;
        }
        ...
        .flip-card-back.active {
          animation: flipIn 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55) reverse forwards;
        }
      ...
      <div
        className={`flip-card-container absolute inset-0 w-full transition-all duration-600 ${
          isFlipped ? 'card-glow flipped' : 'card-glow'
        }`}
      >
        {/* Front Side */}
        {!isFlipped && (
          <button
            className="flip-card-front flex items-center gap-4 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 hover:border-amber-400/50 hover:-translate-y-0.5 transition-all group cursor-pointer w-full h-full"
            type="button"
        ...
        {/* Back Side */}
        {isFlipped && (
          <div className="flip-card-back flex flex-col gap-3 bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-950 border border-amber-400/30 rounded-2xl p-5 w-full h-full justify-center items-center">
            <button
